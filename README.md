...
2. **Fetch Weather Based on Location**: The app automatically retrieves and displays weather information based on your current location using GPS. Make sure location services are enabled on your device.
3. **Search for Specific Locations**: Use the search bar to type in a location. As you type, suggestions for locations will appear based on the autocomplete functionality. Select a suggested location to view its weather information.
...
- [Weather API Documentation](https://openweathermap.org/api) (or whichever API you used)
- [Google Places API](https://developers.google.com/places/web-service/overview) (if used for autocomplete)
